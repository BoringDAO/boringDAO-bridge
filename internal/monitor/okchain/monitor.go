...
func New(config *repo.Config, logger logrus.FieldLogger) (*Monitor, error) {
	storagePath := repo.GetStoragePath(config.RepoRoot, "okchain")
	...
	privKey, err := crypto.ToECDSA(hexutil.Decode(config.OkChain.PrivKey))
	if err != nil {
		return nil, err
	}
	...
	if config.OkChain.GasLimit < 800000 {
		auth.GasLimit = 1500000
	} else {
		auth.GasLimit = config.OkChain.GasLimit
	}

	minConfirms := 0
	if config.OkChain.MinConfirms > 0 {
...
func (m *Monitor) listenLockEvent() {
	...
	err = retry.Retry(func(attempt uint) error {
		filter, err = m.rpcBorBsc.FilterCrossBurn(&bind.FilterOpts{Start: m.bHeight, End: nil, Context: m.ctx})
		if err != nil {
...
func (m *Monitor) handleCross(lock *BorBSCCrossBurn, isHistory bool) {
	if !strings.EqualFold(lock.Raw.Address.String(), m.config.OkChain.BorBscContract) {
		return
	}
...
func (m *Monitor) CrossMint(txId string, addrFromEth common.Address, recipient common.Address, amount *big.Int) error {
	...
	price, err := m.rpcClient.SuggestGasPrice(context.TODO())
	if err != nil {
		return err
	}
	...
	transaction, err := m.rpcSession.CrossMint(addrFromEth, recipient, amount, txId)
	if err != nil {
...
func (m *Monitor) GetLockLog(txId string) (*Coco, error) {
	receipt, err := m.rpcClient.TransactionReceipt(context.TODO(), common.HexToHash(txId))
	if err != nil {
		return nil, err
	}
	for _, log := range receipt.Logs {
		if !strings.EqualFold(log.Address.String(), m.config.OkChain.BorBscContract) {
			continue
		}
...
func (m *Monitor) fetchBlockNum() uint64 {
	header, err := m.rpcClient.BlockNumber(context.TODO())
	if err != nil {
		m.logger.Error(err)
		return 0
	}
...
func (m *Monitor) loadHeightFromStorage() {
	...
	if err = retry.Retry(func(attempt uint) error {
		header, err = m.rpcClient.BlockNumber(context.TODO())
		if err != nil {
			m.logger.Errorf("BlockNumber error:%v", err)
		}
		return err
	...
	if b == nil {
		m.bHeight = header - m.minConfirms
		m.persistBHeight(m.bHeight)
	} else {
		m.bHeight = binary.LittleEndian.Uint64(b)
	}

	if m.config.OkChain.Height != 0 {
		m.bHeight = m.config.OkChain.Height
	}
...
func (m *Monitor) persistBHeight(height uint64) {
	...
	m.logger.WithFields(logrus.Fields{
		"height": m.bHeight,
	}).Info("Persist OkChain Block Height")
}

func TxKey(hash string) []byte {
	return []byte(fmt.Sprintf("okex-tx-%s", hash))
}
